...
frappe.pages['daily-eggs-loading-a'].on_page_load = function(wrapper) {
	new MyPage(wrapper);
}
...
MyPage =Class.extend({
	...
	init: function(wrapper){
			...
			parent: wrapper,
			title: 'Daily Eggs Loading And Stock Report',
			single_column: true
		});
			this.make();
	},
	make: function()
	{
		...
		frappe.call({
          method: 'livestock.poultry.page.daily_eggs_loading_a.daily_egg_stock_report.get_company_list',
          args: {},
          callback: function (r) {
            if (r.message) {
		...
		function get_report()
		{
			...
			{		
					frappe.call({
					method: 'livestock.poultry.page.daily_eggs_loading_a.daily_egg_stock_report.get_egg_report',
					freeze: 1,
